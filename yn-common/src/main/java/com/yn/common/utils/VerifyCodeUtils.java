...
public class VerifyCodeUtils {
    private static final Log log = LogFactory.getLog(VerifyCodeUtils.class);
    ...
    public static String generateVerifyCode(int verifySize, String sources) {
        ...
        // Random rand = new Random(SystemClock.now());
        StringBuilder verifyCode = new StringBuilder(verifySize);
        for (int i = 0; i < verifySize; i++) {
            verifyCode.append(sources.charAt(RandomUtil.getRandom().nextInt(codesLen - 1)));
        }
    ...
    private static int[] getRandomRgb() {
        int[] rgb = new int[3];
        for (int i = 0; i < 3; i++) {
            rgb[i] = RandomUtil.getRandom().nextInt(255);
        }
        return rgb;
    }
    ...
    private static void shearY(Graphics g, int w1, int h1, Color color) {
        ...
        int period = RandomUtil.getRandom().nextInt(40) + 10; // 50;
